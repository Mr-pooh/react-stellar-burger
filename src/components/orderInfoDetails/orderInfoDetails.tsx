...
const OrderInfoDetails: FC = () => {
  ...
  const { details } = useAppSelector(getStoreModalIngredient);
  ...
  const arr = React.useMemo(() => {
    if (details?.ingredients) {
      ...
      details.ingredients.forEach((item: TIngredient | string) => {
        data.forEach((elem) => {
          if (elem._id === item) {
            array.push(elem);
          }
  ...
  const totalPrice = React.useMemo(
    () => arr && arr.reduce((a, b) => a + b.price, 0),
    [arr]
  );
  ...
  const ingredient = React.useCallback(() => {
    if (details?.ingredients) {
      ...
      return arrIngr.map((item) => {
        const sum = 0;
        const amount = arr?.reduce((a: number, b: TIngredient) => {
          if (b === item) {
            a = a + 1;
          }
          return a;
